...
class EditSettingsForm(UserChangeForm):
    ...
    last_login = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'class': 'form-control', 'style': 'background:none; border:none'}))
    date_joined = forms.CharField(max_length=100, widget=forms.TextInput(attrs={'class': 'form-control', 'style': 'background:none; border:none'}))

    class Meta:
        model = User
        fields = ('username', 'first_name', 'last_name', 'email', 'is_staff', 'is_active', 'last_login', 'date_joined', 'password')
...
class PasswordChangingForm(PasswordChangeForm):
    ...
    new_password1 = forms.CharField(widget=forms.PasswordInput(attrs={'class': 'form-control', 'type':'password', 'label': 'New Password Change'}))
